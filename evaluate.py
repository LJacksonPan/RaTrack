...
from vod.configuration.file_locations import KittiLocations, VodTrackLocations
from vod.frame import FrameDataLoader, FrameTransformMatrix; matplotlib.use('Agg')
...
import mailpy
from AB3DMOT_libs.dist_metrics import iou, iou_points
...
def boxoverlap(a, b, criterion="union", piou=False):
    """
...
def get_bbx_param(obj_info, transforms, sensor):
    ...
    if sensor == 'lidar':
        center = (transforms.t_lidar_camera @ np.array([obj_info.x, obj_info.y, obj_info.z, 1]))[:3]
    if sensor == 'radar':
        center = (transforms.t_radar_camera @ np.array([obj_info.x, obj_info.y, obj_info.z, 1]))[:3]
    # enlarge the box field to include points with meansure errors
    extent = np.array([obj_info.l, obj_info.w, obj_info.h]) # l w h
    angle = [0, 0, -(obj_info.ry + np.pi / 2)]
...
class trackingEvaluation(object):
    ...
    def __init__(self, t_sha, gt_path="./scripts/KITTI", max_truncation = 1, min_height = 25, max_occlusion = 2, \
        mail=None, cls="car", eval_3diou=True, eval_piou=False, eval_2diou=False, num_hypo=1, thres=None):
        # get number of sequences and
        # get number of frames per sequence from test mapping
        # (created while extracting the benchmark)
        # filename_test_mapping = os.path.join('evaluate_tracking.seqmap.val')
        self.n_frames         = [] # number of frames in each sequnce
        self.sequence_name    = []
        ...
        for clip in clips_list:
            ...
            if clip_name not in val_seq:
                continue
            self.sequence_name.append(clip.split('/')[-1].split('.')[0])
            with open(clip, 'r') as f:
                lines = f.readlines()
            self.n_frames.append(len(lines))


        self.n_sequences = len(val_seq)
        ...
        # data and parameter
        self.gt_path           = os.path.join('./vod_tracking_gt')
        self.t_sha             = t_sha
        # export PYTHONPATH=${PYTHONPATH}:/home/zhijun/AB3DMOT
        self.t_path            = os.path.join(result_path)
        # self.t_path            = os.path.join('./results/track4d_results/')
        print('Tracking path: ', self.t_path)
        # self.t_path            = os.path.join('../../vod_tracking_gt/')
    ...
    def loadGroundtruth(self):
        ...
        try:
            self._loadData(self.gt_path, cls=self.cls, loading_groundtruth=True, point_data=True)
        except IOError:
            return False
        return True
    ...
    def loadTracker(self):
        ...
        try:
            if not self._loadData(self.t_path, cls=self.cls, loading_groundtruth=False, point_data=True):
                return False
        except IOError:
            return False
        return True

    def _loadData(self, root_dir, cls, min_score=-1000, loading_groundtruth=False, point_data=False):
        """
        ...
        for seq, s_name in enumerate(self.sequence_name):
            path = os.path.join(root_dir, s_name)
            result_list = sorted(glob.glob(path + '/*.txt'))
            ...
            clips_file = os.path.join('./clips/', s_name+'.txt')
            with open(clips_file, 'r') as f:
                first_frame_number = int(f.readline().strip())
            ...
            for result in result_list:
                if f_idx < 2:
                    f_idx += 1
                    continue
                frame_number = result.split('/')[-1].split('.')[0]
                ...
                for line in lines:
                    ...
                    if not point_data:
                        if "car" in cls.lower():
                            ...
                            classes = [cls.lower()]
                        classes += ["dontcare"]
                        if not any([s for s in classes if s in fields[0].lower()]):
                            continue
                    # load into traker class
                    if loading_groundtruth:
                        t_data.frame = int(frame_number) - first_frame_number # frame
                        t_data.track_id = int(float(fields[1]))  # id
                        if t_data.track_id in line_to_skip:
                            continue
                        t_data.obj_type = fields[0].lower()  # object type [car, pedestrian, cyclist, ...]
                        t_data.truncation = -1  # truncation [-1,0,1,2]
                        t_data.occlusion = int(float(fields[2]))  # occlusion  [-1,0,1,2]
                        t_data.obs_angle = float(fields[3])  # observation angle [rad]
                        t_data.x1 = float(fields[4])  # left   [px]
                        t_data.y1 = float(fields[5])  # top    [px]
                        t_data.x2 = float(fields[6])  # right  [px]
                        t_data.y2 = float(fields[7])  # bottom [px]
                        ...
                        # t_data.z = float(fields[13]) + t_data.h/2  # Z [m]
                        t_data.z = float(fields[13]) # Z [m]
                        t_data.ry = float(fields[14])  # yaw angle [rad]
                        ...
                        if point_data:
                            ...
                            frame_data_new = FrameDataLoader(kitti_locations=kitti_locations,
                                                        frame_number=str(frame_number).zfill(5))
                            transforms = FrameTransformMatrix(frame_data_new)
                            raw_pc = np.array(frame_data_new.radar_data[:, :3])

                            box = get_bbx_param(t_data, transforms, 'radar')
                            pc_ = o3d.utility.Vector3dVector(raw_pc)
                            in_box_idx = box.get_point_indices_within_bounding_box(pc_)
                            pc = raw_pc[in_box_idx]
                            t_data.points = np.array(pc)

                            if t_data.points.shape[0] < omit_p:
                                continue

                            mov = int(frame_data_new.raw_detection_labels[line_idx].split(' ')[1])
                            if mov != 1 and mov_only:
                                continue
                            # print(pc.shape)
                            t_data.truncation = mov

                            if t_data.obj_type == 'rider':
                                # combine rider
                                best_dist = 99999999
                                best_match = None
                                for line_ in lines:
                                    fields_ = line_.split(' ')
                                    if int(float(fields_[1])) == t_data.track_id:
                                        continue
                                    p1 = np.array([float(fields_[11]), float(fields_[12]), float(fields_[13])])
                                    p2 = np.array([t_data.x, t_data.y, t_data.z])
                                    dist = np.linalg.norm(p1 - p2)
                                    if dist < best_dist:
                                        best_match = fields_
                                
                                t_data_ = tData()
                                t_data_.x1 = float(best_match[4])  # left   [px]
                                t_data_.y1 = float(best_match[5])  # top    [px]
                                t_data_.x2 = float(best_match[6])  # right  [px]
                                t_data_.y2 = float(best_match[7])  # bottom [px]
                                t_data_.h = float(best_match[8])  # height [m]
                                t_data_.w = float(best_match[9])  # width  [m]
                                t_data_.l = float(best_match[10])  # length [m]
                                t_data_.x = float(best_match[11])  # X [m]
                                t_data_.y = float(best_match[12])  # Y [m]
                                # t_data_.z = float(best_match[13]) + t_data_.h/2  # Z [m]
                                t_data_.z = float(best_match[13]) # Z [m]
                                t_data_.ry = float(best_match[14])  # yaw angle [rad]

                                box = get_bbx_param(t_data_, transforms, 'radar')
                                pc_ = o3d.utility.Vector3dVector(raw_pc)
                                in_box_idx = box.get_point_indices_within_bounding_box(pc_)
                                pc = raw_pc[in_box_idx]

                                t_data.points = np.concatenate((t_data.points, np.array(pc)), 0)
                                line_to_skip.append(int(float(fields_[1])))
                            
                    elif point_data and not ab3d:
                        t_data.frame = int(frame_number) - first_frame_number # frame
                        t_data.obj_type = fields[0].lower()  # object type [car, pedestrian, cyclist, ...]
                        ...
                        t_data.points = []
                        for i in range(int(len(fields[6:]) / 3)):
                            t_data.points.append([float(fields[6:][i * 3 + 0]), 
                                                float(fields[6:][i * 3 + 1]), 
                                                float(fields[6:][i * 3 + 2])])
                        t_data.points = np.array(t_data.points)
                        # print(t_data.points.shape)
                    elif point_data and ab3d:
                        ...
                        raw_pc = np.array(frame_data.radar_data[:, :3])
    ...
    # def compute3rdPartyMetrics(self, threshold=3):
        ...
        for seq_idx in range(len(self.groundtruth)):
            ...
            for f in range(len(seq_gt)):        # go through each frame
                ...
                for gg in g:
                    ...
                    for tt in t:
                        if self.eval_2diou:
                            ...
                            # TODO: stopped here
                            c = 1 - iou_points(gg, tt)
                            # if c < 1:
                            #     print()
                        else:
                            assert False, 'error'

                        # if iou(gg, tt, metric='iou_3d') > 0.5:
                            # print('iou: ', iou(gg, tt, metric='iou_3d'))
                ...
                # Moved up to ignore static obj for ab3d
                ...
                for row,col in association_matrix:
                    ...
                    # For AB3D, ignore associated static objects by label
                    # if g[row].truncation != 1:
                    #     g_to_remove.append(g[row])
                    #     t_to_remove.append(t[col])
                    #     amat_to_remove.append((row,col))
                    #     ids_to_remove.append(g[row].track_id)
                    #     continue
                ...
                for ggg in g_to_remove:
                    g.remove(ggg)
                for ttt in t_to_remove:
                    t.remove(ttt)
                for amat in amat_to_remove:
                    association_matrix.remove(amat)
                for id in ids_to_remove:
                    idx = this_ids[0].index(id)
                    this_ids[0].remove(id)
                    this_ids[1].pop(idx)
                ...
                # self.n_gt -= len(g_to_remove)
                # self.n_tr -= len(t_to_remove)
                
                # n_gts -= len(g_to_remove)
                # n_trs -= len(t_to_remove)
...
class stat:
    ...
    def plot(self):
        save_dir = os.path.join("./results/vod", self.t_sha)
...
def evaluate(result_sha,mail,num_hypo,eval_3diou,eval_piou,eval_2diou,thres):
    """
    ...
    for c in ("a"):
    # for c in ["pedestrian"]:
        e = trackingEvaluation(t_sha=result_sha, mail=mail,cls=c,eval_3diou=eval_3diou,eval_piou=eval_piou,eval_2diou=eval_2diou,num_hypo=num_hypo,thres=thres)
...
if __name__ == "__main__":
    ...
    success = evaluate(result_sha,mail,num_hypo,False,True,eval_2diou,thres)