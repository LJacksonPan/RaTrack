    ...
    for epoch in range(args.epochs):
        ...
        if best_test_loss >= test_loss:
            ...
            if torch.cuda.device_count() > 1:
                torch.save(net.module.state_dict(), 'checkpoints/%s/models/model.bestv2.t7' % args.exp_name)
            else:
                torch.save(net.state_dict(), 'checkpoints/%s/models/model.bestv2.t7' % args.exp_name)
    ...
    if args.model == 'dcp':
        ...
        if args.eval:
            if args.model_path is '':
                model_path = 'checkpoints' + '/' + args.exp_name + '/models/model.bestv2.t7'
            else:
