...
from vod import FrameDataLoader, FrameTransformMatrix, KittiLocations
...
class AB3DMOT(object):
    ...
    def ego_from_vod(self, frame, x, y, z):
        ...
        for i in range(0, 10):
            try:
                frame_data1 = FrameDataLoader(kitti_locations=kitti_locations,
                                              frame_number=str(frame + i).rjust(5, '0'))
                transforms1 = FrameTransformMatrix(frame_data1)
                frame_data2 = FrameDataLoader(kitti_locations=kitti_locations,
                                              frame_number=str(frame + i + 1).rjust(5, '0'))
                transforms2 = FrameTransformMatrix(frame_data2)

                odom_cam_1 = transforms1.t_odom_camera
                odom_cam_2 = transforms2.t_odom_camera

                odom1_odom2 = np.dot(np.linalg.inv(odom_cam_1), odom_cam_2)

                xyz_comp = np.dot(np.linalg.inv(odom1_odom2), np.array([x, y, z, 1]))[:3]
                return np.array([xyz_comp])
            except Exception as e:
                continue
        raise IndexError("No ego motion found")
    ...
    def ego_motion_compensation(self, frame, trks):
        ...
        for index in range(len(self.trackers)):
            ...
            # compensated = egomotion_compensation_ID(xyz, self.calib, ego_rot_imu, ego_xyz_imu, left, right)  # todo replace here with fangqiang method
            compensated = self.ego_from_vod(frame, trk_tmp.x, trk_tmp.y, trk_tmp.z)
