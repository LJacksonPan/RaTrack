...
def box2corners3d_camcoord_vod(box3d:Box3D, transformations_matrix):
    # get corners
    x_corners = [box3d.l / 2,
                 box3d.l / 2,
                 -box3d.l / 2,
    ...
    rotation = -(box3d.ry + np.pi / 2)
    rot_matrix = np.array([[np.cos(rotation), -np.sin(rotation), 0],
                           [np.sin(rotation), np.cos(rotation), 0],
                           [0, 0, 1]])
    center = (transformations_matrix.t_lidar_camera @ np.array([box3d.x,
                                                                box3d.y,
                                                                box3d.z,
                                                                1]))[:3]
    new_corner_3d = np.dot(rot_matrix, corners_3d).T + center
...
def vis_obj(box, img, calib, hw, color_tmp=None, str_vis=None, thickness=4, id_hl=None, err_type=None, frame_transform=None):
    ...
    # draw box
    obj_8corner = box2corners3d_camcoord_vod(box, frame_transform)
    # return 8, 3 array
    ...
    # _, _ = draw_box3d_image(img, new_obj_pts_2d, hw, color=(51, 0, 255), thickness=thickness)
